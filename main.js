...
const CURRENT_UI_VERSION = 1.2; // <--- ALTERADO DE 1.0 PARA 1.2
...
import { 
    ...
    orderBy, 
    getDocs,
    doc,       // Adicionado para obter um documento específico (settings)
    getDoc     // Adicionado para obter um documento específico (settings)
...
async function checkForForcedUpdate() {
    try {
        // Busca o documento 'config' dentro da coleção 'settings'
        const settingsDocRef = doc(db, 'settings', 'config'); 
        const docSnap = await getDoc(settingsDocRef);
        
        if (docSnap.exists()) {
            // Se o campo não existir, assume 1.0. Se existir, usa o valor do servidor.
            const serverVersion = docSnap.data().web_ui_version || 1.0; 
            
            // Compara a versão do servidor (controlada pelo Admin) com a versão do código local.
            if (serverVersion > CURRENT_UI_VERSION) {
                ...
                // Exibe um modal/prompt para melhor UX
                if (confirm("Uma atualização de sistema essencial está disponível. Recarregar agora para ver as novidades?")) {
                    // window.location.reload(true) força o navegador a IGNORAR o cache
                    window.location.reload(true); 
                } else {
                    // Se o usuário não aceitar, força o reload após um pequeno delay para garantir a atualização.
...
// --- 4. NOVAS FUNÇÕES DE RENDERIZAÇÃO DE LAYOUT ---
...
// NOVO: Renderiza o layout principal dinâmico (Feed misturado + Seções Injetadas)
...
function showDetailsModal(itemData) {
    ...
    if (relatedItems.length > 0) {
        ...
        // Reutiliza o renderAppCard que cria o card horizontal por padrão
...
document.querySelectorAll('.nav-menu a').forEach(link => {
    link.addEventListener('click', (event) => {
        ...
        // O link do WhatsApp é tratado pelo target="_blank"
    });
});
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await checkForForcedUpdate(); // <--- CHAMA A CHECAGEM DE VERSÃO APÓS CARREGAR O CONTEÚDO
